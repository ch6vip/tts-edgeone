...
## ⚡ 最新更新 (v2.5.1 - 2026-01-15)

### 🔄 回退 ES6 模块，保留所有性能优化

**变更原因**：为了更好的 EdgeOne Pages 兼容性，回退到不使用 ES6 模块的版本。

**重要说明**：
- ✅ **所有性能优化均已保留**（流式写入、Token 竞态保护、文本分块优化等）
- ✅ 移除了 `import/export` 语句，改用内联工具函数
- ✅ 可以删除 `edge-functions/utils/` 目录（已不再使用）
- ✅ 代码逻辑完全相同，仅是模块化方式的调整

**修改的文件**：
1. `edge-functions/api/v1/audio/speech.js` - 内联所有工具函数
2. `edge-functions/api/v1/models.js` - 内联所有工具函数

**不再需要的文件（可删除）**：
- `edge-functions/utils/cors.js`
- `edge-functions/utils/errors.js`
- `edge-functions/utils/text-processing.js`
- `edge-functions/utils/base64.js`
...
### 主要文件（2 个）
1. `edge-functions/api/v1/audio/speech.js` - TTS 核心逻辑（包含所有优化）
2. `edge-functions/api/v1/models.js` - 模型列表接口
3. `index.html` - 前端界面

### ⚠️ 可删除的文件（已废弃）
1. ~~`edge-functions/utils/cors.js`~~ - 已内联到主文件
2. ~~`edge-functions/utils/errors.js`~~ - 已内联到主文件
3. ~~`edge-functions/utils/text-processing.js`~~ - 已内联到主文件
4. ~~`edge-functions/utils/base64.js`~~ - 已内联到主文件

## ⚠️ 注意事项

1. **模块化方式**：已回退到不使用 ES6 `import/export`，使用内联函数
2. **atob/btoa**：简化后的 Base64 处理依赖这两个函数
3. **向后兼容**：API 接口保持不变，现有配置无需迁移
4. **性能保证**：所有性能优化逻辑均已保留，性能表现与 v2.5.0 相同
...
**版本历史：**
- v2.5.1 (2026-01-15) - 回退 ES6 模块，保留所有性能优化
- v2.5.0 (2026-01-15) - 初始优化版本（使用 ES6 模块）
